...
# %% auto 0
__all__ = ['connect_db', 'clean_mosaico', 'update_radcom', 'update_stel', 'split_designacao', 'update_srd', 'update_telecom',
           'update_aero', 'validar_coords', 'update_cached_df', 'update_base']
...
def split_designacao(df: pd.DataFrame)->pd.DataFrame:
    ...
    df["DesignacaoEmissao"] = df.DesignacaoEmissao.str.replace(",", " ").str.strip().str.upper()
    df["DesignacaoEmissao"] = df.DesignacaoEmissao.str.split(" ")
    df = df.explode("DesignacaoEmissao")
    df = df[df.DesignacaoEmissao != "/"]  # Removes empty rows
    df[["Largura_Emissão(kHz)", "Classe_Emissão"]] = df.DesignacaoEmissao.apply(parse_bw).tolist()
    df[["Largura_Emissão(kHz)", "Classe_Emissão"]] = df[["Largura_Emissão(kHz)", "Classe_Emissão"]].astype('string')
    return df.drop("DesignacaoEmissao", axis=1)
